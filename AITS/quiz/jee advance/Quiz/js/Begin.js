...
function CheckResult() {
    ...
    $(".test-questions").find("li").each(function (ind) {
        ...
        if (r.children().hasClass("que-save") || r.children().hasClass("que-save-mark")) {
            ...
            if ((ind >= 14 && ind <= 17) || (ind >= 32 && ind <= 35) || (ind >= 50 && ind <= 53)) {
            ...
            else if ((ind >= 6 && ind <= 13) || (ind >= 24 && ind <= 31) || (ind >= 42 && ind <= 49)) {
                ...
                var seloption = "";
                $("#" + a).find("input[name='checkbox" + a + "']").each(function () {
                    var e = $(this);
                    if (e.is(':checked')) {
                        seloption = seloption.concat(e.val());
                        selectedAns = selectedAns.concat(e.val(), ",");
                        if (currectAns.indexOf(e.val()) > -1) {
                            mcscore++;
                        }
                        else
                            wrong++;
                ...
                if ((seloption.length == currectAns.length) && (mcscore == currectAns.length))
                    ansStatus = "Correct";
                else if ((seloption.length < currectAns.length) && (mcscore > 0 && wrong == 0))
                    ansStatus = "Partial Correct";
                selectedAns = selectedAns.slice(0, selectedAns.length - 1);
            }

            if (ansStatus == 'Correct') {
                if ((ind >= 0 && ind <= 5) || (ind >= 18 && ind <= 23) || (ind >= 36 && ind <= 41))
                    score = score + 4;
                else
                    score = score + 3;
                TotalCorrect = TotalCorrect + 1;
            } else if (ansStatus == "Wrong") {
                if ((ind >= 14 && ind <= 17) || (ind >= 32 && ind <= 35) || (ind >= 50 && ind <= 53))
                    score = score - 1;
                else if ((ind >= 0 && ind <= 5) || (ind >= 18 && ind <= 23) || (ind >= 36 && ind <= 41))
                    score = score - 2;
        ...
        if (r.children().hasClass("que-save") || r.children().hasClass("que-save-mark")) {
            if (ansStatus == 'Correct') {
                ...
                tr.append('<td><span class="label label-warning">' + ansStatus + '</span></td>');
            }
        } else {
            tr.append('<td>N/A</td>');
        }
        if ((ind >= 0 && ind <= 5) || (ind >= 18 && ind <= 23) || (ind >= 36 && ind <= 41)) {
            var arr = currectAns.split("");
            var cans = "";
            for (var x = 0; x < arr.length; x++)
                cans = cans.concat(arr[x], ",");
            cans = cans.slice(0, cans.length - 1);
            tr.append('<td>' + cans + '</td>');
        }
        else
    ...
    $('#lblRTotalPartCorrect').text(TotalPartial);
    ...
    if (!e.shiftKey) {
        ...
        if (charCode == 190 || charCode == 110) {
            ...
            if (charCode > 31 && (charCode < 48 || charCode > 57)) {
                if (charCode == 37 || charCode == 39 || (charCode > 95 && charCode < 106) || charCode == 189 || charCode == 109) {
...
function initializeKeypad() {
    ...
    while (i <= 50) {
        for (var j = 0; j < 8; j++) {
            ...
            $('.inlinepage' + ind).on('keydown', checkValue);
            i++;
        }
        i += 10;
    }
...
$(document).ready(function () {
    ...
    $(".btn-save-answer").click(function (e) {
        ...
        if ((ind >= 14 && ind <= 17) || (ind >= 32 && ind <= 35) || (ind >= 50 && ind <= 53)) {
            ...
            if ($("input[name='checkbox" + a + "']").each(function () {
                $(this).is(":checked") && (n = !0)
            }), 0 == n) { alert("Please choose an option or options"); return !1 };
            $("input[name='checkbox" + a + "']:checked").val(), t.find("a").removeClass("que-save-mark"), t.find("a").removeClass("que-mark"), t.find("a").addClass("que-save"), t.find("a").removeClass("que-not-answered"), t.find("a").removeClass("que-not-attempted"), NextQuestion(!1), CheckQueAttemptStatus()
        }
    });
    $(".btn-save-mark-answer").click(function (e) {
        ...
        if ((ind >= 14 && ind <= 17) || (ind >= 32 && ind <= 35) || (ind >= 50 && ind <= 53)) {
            ...
            $("input[name='checkbox" + a + "']:checked").val(), t.find("a").removeClass("que-save"), t.find("a").removeClass("que-mark"), t.find("a").addClass("que-save-mark"), t.find("a").removeClass("que-not-answered"), t.find("a").removeClass("que-not-attempted"), NextQuestion(!1), CheckQueAttemptStatus()
        }
    });
    ...
    function generate(final, userid, qnaTable) {
        XlsxPopulate.fromBlankAsync()
            .then(workbook => {
                ...
                for (var j = 0; j < qnaTable.length; j++) {
                    ...
                    for (key in qnaTable[j]) {
                        ...
                        if (key == "Status") {
                            ...
                            else if (qnaTable[j][key] == "Partial Correct")
                                r.style("fill", "ffaa00");
                            else
                                r.style("fill", "2f74b5");
                        }
                ...
                return workbook.outputAsync({ type: "blob", password: "blue_sky.secret38" })
                    .then(function (blob) {
                        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
                            window.navigator.msSaveOrOpenBlob(blob, userid1 + "-" + date + ".xlsx");
                        } else {
                            var url = window.URL.createObjectURL(blob);
                            var a = document.createElement("a");
                            document.body.appendChild(a);
                            a.href = url;
                            a.download = userid1 + "-" + date + ".xlsx";
                            a.click();
    ...
    function downloadResult() {
        ...
        if (user != null) {
            ...
            final.push({
                ...
                "Partial Correct": rows[2].col1,
                "Score": $('#lblRScore').text(),
                "Total Time Taken": rows[2].col2
            });
