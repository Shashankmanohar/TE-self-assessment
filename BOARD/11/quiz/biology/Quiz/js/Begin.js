...
function CheckResult() {
    ...
    $(".test-questions").find("li").each(function (ind) {
        ...
        if (r.children().hasClass("que-save") || r.children().hasClass("que-save-mark")) {
           ...
           if ((ind <= 180) || (ind > 194 && ind <= 1144) || (ind > 1149 && ind <= 1169)) {
...
$(document).ready(function () {
    ...
    $(".btn-save-answer").click(function (e) {
        ...
        var ind = t.index();
        if ((ind <= 180) || (ind > 1124 && ind <= 1144) || (ind > 1149 && ind <= 1169)) {
                ...
                return workbook.outputAsync({ type: "blob", password: "bluesky" })
                    .then(function (blob) {
                        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
                            window.navigator.msSaveOrOpenBlob(blob, userid1 + "-" + date + ".1xlsx");
                        } else {
                            var url = window.URL.createObjectURL(blob);
                            var a = document.createElement("a");
                            document.body.appendChild(a);
                            a.href = url;
                            a.download = userid1 + "-" + date + ".1xlsx";
                            a.click();
